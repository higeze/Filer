...
void CTaskRow::RenderBackground(CDirect2DWrite* pDirect, const CRectF& rc)
{
	...
	switch (task.State->get_const()) {
		...
		default:
			if (task.YearMonthDay->get_const().IsInvalid()) {
				//Transparent
			} else {
				...
				if (CYearMonthDay::Today() == task.YearMonthDay->get_const()) {
					pDirect->FillSolidRectangle(today, rc);
				} else if(CYearMonthDay::Tomorrow() == task.YearMonthDay->get_const()){
					pDirect->FillSolidRectangle(tomorrow, rc);
				} else if(CYearMonthDay::Today() > task.YearMonthDay->get_const()){
