...
{
	if (auto pBindRow = dynamic_cast<CBindRow*>(this->m_pRow)) {
...
std::wstring CFavoriteCell::GetString()
{
	if (auto pBindRow = dynamic_cast<CBindRow*>(this->m_pRow)) {
		if (GetShellFile()) {
...
std::shared_ptr<CShellFile> CFavoriteCell::GetShellFile()
{
	if (auto pBindRow = dynamic_cast<CBindRow*>(this->m_pRow)) {
		return pBindRow->GetItem<std::shared_ptr<CFavorite>>()->GetShellFile(
			[wp = std::weak_ptr(std::dynamic_pointer_cast<CFavoriteCell>(this->shared_from_this()))]() {
				if (auto sp = wp.lock()) {
...
void CFavoriteCell::PaintContent(CDirect2DWrite* pDirect, CRectF rcPaint)
{
	//Paint Icon
...
void CFavoriteCell::OnContextMenu(const ContextMenuEvent& e)
{
	...
	if (retID == CResourceIDFactory::GetInstance()->GetID(ResourceType::Command, L"EditFavorite")) {
		...
		auto pCol = static_cast<const CFavoritesColumn*>(this->m_pColumn);
		auto& itemsSource = static_cast<CFavoritesGridView*>(this->m_pSheet)->ItemsSource;
