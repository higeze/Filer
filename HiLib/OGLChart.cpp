...
void COGLChart::AddSeries(std::shared_ptr<COGLSeries> spSeries)
{
	...
	//Calc Min Max Value
	std::for_each(spSeries->m_mVertex.origin(),spSeries->m_mVertex.origin()+spSeries->m_mVertex.num_elements(),[&](CVertex& vertex)->void
	{
...
{
	return	(value-m_spYAxis->GetMinimum())/(m_spYAxis->GetMaximum()-m_spYAxis->GetMinimum())*m_spYAxis->GetUnit();
}
...
{
	return CVertex(
		(vertex.x-m_spXAxis->GetMinimum())/(m_spXAxis->GetMaximum()-m_spXAxis->GetMinimum())*m_spXAxis->GetUnit(),
		(vertex.y-m_spYAxis->GetMinimum())/(m_spYAxis->GetMaximum()-m_spYAxis->GetMinimum())*m_spYAxis->GetUnit(),
		(vertex.z-m_spZAxis->GetMinimum())/(m_spZAxis->GetMaximum()-m_spZAxis->GetMinimum())*m_spZAxis->GetUnit());
}
