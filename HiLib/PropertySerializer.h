...
public:
	...
	void operator()(const char_type* lpszName,T& t)
	{
		...
		m_pSheet->Cell(pItemRow, pColName)->SetStringCore(std::wstring(lpszName,(lpszName+strlen(lpszName))));
	...
	{
		//m_pSheet->Cell(pRow, pCol) = //->SetStringCore(boost::lexical_cast<std::wstring>(t));
	...
	{
		//m_pSheet->Cell(pRow, pCol) = //->SetStringCore(str2wstr(t));
	...
	{
		//m_pSheet->Cell(pRow, pCol)=std::make_shared<CColorCell>(
		//	m_pSheet,
		//	pRow,
		//	pCol,
	...
	{
		//m_pSheet->Cell(pRow, pCol)=std::make_shared<CFontCell>(
		//	m_pSheet,
		//	pRow,
		//	pCol,
		//	m_pSheet->GetCellProperty(),
	...
	{
		//m_pSheet->Cell(pRow, pCol)=std::make_shared<CBoolCell>(
		//	m_pSheet,
		//	pRow,
		//	pCol,
		//	m_pSheet->GetCellProperty(),
	...
	void SerializeValue(T& t,CRow* pRow,CColumn* pCol)
	{
		m_pSheet->Cell(pRow, pCol)->SetStringCore(boost::lexical_cast<std::wstring>(static_cast<int>(t)));
	}
	...
	void SerializeValue(T& t,CRow* pRow,CColumn* pCol)
	{
		...
		m_pSheet->Cell(pRow, pCol)=spSheetCell;
	...
	void SerializeValue(ReactiveProperty<std::tuple<T>>& t, CRow* pRow, CColumn* pCol)
	{
		auto spCell = std::make_shared<CBindPropertySheetCell<T>>(
			...
			m_pSheet->GetCellProperty(),
			t);
		m_pSheet->Cell(pRow, pCol) = spCell;
		spCell->UpdateAll();
	}
	//For shared_ptr
	template<class T>
...
//		m_setCellPtr.clear();
	...
	//				auto pCell = Cell(rowIter->DataPtr, pCol);
	...
	void operator()(const char_type* lpszName, T& t, U... args)
	{
		if (!m_pSheet->Empty()) {
			...
			for (auto rowIter = m_pSheet->GetContainer<RowTag, AllTag>().begin() + m_pSheet->GetFrozenCount<RowTag>(), rowEnd = m_pSheet->GetContainer<RowTag, AllTag>().end(); rowIter != rowEnd; ++rowIter) {
				//auto pCell = m_pSheet->Cell(*rowIter, pCol);
				if (/*m_setCellPtr.find(pCell.get()) == m_setCellPtr.end() && */m_pSheet->Cell(*rowIter, spNameCol)->GetString() == wstrName) {
					//m_setCellPtr.insert(pCell.get());
	...
	{
		t=boost::lexical_cast<T>(m_pSheet->Cell(pRow, pColumn)->GetString());
	}
	...
	{
		t=wstr2str(m_pSheet->Cell(pRow, pColumn)->GetString());
	}

	//For Color
	void DeserializeValue(CColorF& t, CRow* pRow, CColumn* pColumn)
	{
		if(auto p=std::dynamic_pointer_cast<CColorCell>(m_pSheet->Cell(pRow, pColumn))){
			t=p->GetColor();
		}
	}
	...
	{
		if(auto p=std::dynamic_pointer_cast<CFontCell>(m_pSheet->Cell(pRow, pColumn))){
			t=p->GetFont();
		}
	}	
	//For enum
	...
	void DeserializeValue(T& t,CRow* pRow,CColumn* pColumn)
	{
		t=static_cast<T>(boost::lexical_cast<int>(m_pSheet->Cell(pRow, pColumn)->GetString()));
	}
	...
	void DeserializeValue(T& t,CRow* pRow,CColumn* pColumn)
	{
		if(auto pSheet=std::dynamic_pointer_cast<CSheet>(m_pSheet->Cell(pRow, pColumn))){
	...
	void DeserializeValue(ReactiveProperty<std::tuple<T>>& t, CRow* pRow, CColumn* pColumn)
	{
		...
		//if (auto spSheet = std::dynamic_pointer_cast<CSheet>(m_pSheet->Cell(pRow, pColumn))) {
		...
		if (auto spSheet = std::dynamic_pointer_cast<CBindPropertySheetCell<T>>(m_pSheet->Cell(pRow, pColumn))) {
