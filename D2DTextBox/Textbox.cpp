...
CTextBox::CTextBox(
	...
	:CD2DWControl(pParentControl),
	m_initText(text),
	m_pTextLayout(std::make_unique<CTextLayout>(this)),
	EnterText(text),
...
{
	//m_pText->SetWidth(availableSize.width);
	if (Text->empty()) {
...
void CTextBox::Clear()
{
	...
	m_pVScroll->SetScrollRange(0.f, m_pTextLayout->GetHeight());

	m_pHScroll->SetScrollPage(GetPageRect().Width());
	m_pHScroll->SetScrollPos(0.f);
	m_pHScroll->SetScrollRange(0.f, m_pTextLayout->GetWidth());
}
...
{
	Caret.get_unconst()->Select(selectedBegin, selectedEnd, m_pTextLayout->HitTestTextPosition(Text->size()).CenterPoint());
	if (GetIsFocused()) {
		StartCaretBlink();
	}
}
...
void CTextBox::EnsureVisibleCaret()
{
	if (m_isScrollable) {
		if (!Text->empty()) {
			...
			if ((curRect.top - m_pVScroll->GetScrollPos()) < 0) {
				m_pVScroll->SetScrollPos(curRect.top);
			} else if ((curRect.bottom - m_pVScroll->GetScrollPos()) > m_pVScroll->GetScrollPage()) {
				m_pVScroll->SetScrollPos(curRect.bottom - m_pVScroll->GetScrollPage());
			}

			if ((curRect.left - m_pHScroll->GetScrollPos()) < 0) {
				m_pHScroll->SetScrollPos(curRect.left);
			} else if ((curRect.right - m_pHScroll->GetScrollPos()) > m_pHScroll->GetScrollPage()) {
				m_pHScroll->SetScrollPos(curRect.right - m_pHScroll->GetScrollPage());
			}

			//if ((curRect.top - m_pVScroll->GetScrollPos() + ContentRect().top) < ContentRect().top) {
			//	m_pVScroll->SetScrollPos(curRect.top - orgRect.top);
			//} else if ((curRect.bottom - m_pVScroll->GetScrollPos() + ContentRect().top) > ContentRect().bottom) {
			//	m_pVScroll->SetScrollPos(curRect.bottom - orgRect.top - ContentRect().Height());
			//}

			//if ((curRect.left - m_pHScroll->GetScrollPos() + ContentRect().left) < ContentRect().left) {
			//	m_pHScroll->SetScrollPos(curRect.left - orgRect.left);
			//} else if ((curRect.right - m_pHScroll->GetScrollPos() + ContentRect().left) > ContentRect().right) {
			//	m_pHScroll->SetScrollPos(curRect.right - orgRect.left - ContentRect().Width());
			//}
		}
	}
}
...
void CTextBox::Normal_Paint(const PaintEvent& e)
{
	...
	{
		//Translation On
		D2D1::Matrix3x2F translation = D2D1::Matrix3x2F::Translation(ContentRect().left - m_pHScroll->GetScrollPos(), ContentRect().top - m_pVScroll->GetScrollPos());
		GetWndPtr()->GetDirectPtr()->GetD2DDeviceContext()->SetTransform(translation);
		if (!Text->empty()) {
			PaintText(e);
			PaintSelection(e);
			PaintHighlite(e);
...
void CTextBox::Normal_KeyDown(const KeyDownEvent& e)
{
	...
	case VK_LEFT:
	{
		...
		auto rect = m_pTextLayout->HitTestCaretTextPosition(position);
		auto point = CPointF(rect.left, (rect.top + rect.bottom) / 2.f);
	...
	case VK_UP:
	{
		auto curCharRect = m_pTextLayout->HitTestCaretTextPosition(*Caret->Current);
		auto caret_point = *Caret->Point;
		auto point = CPointF(caret_point.x,
			(std::max)(m_pTextLayout->HitTestCaretTextPosition(0).CenterPoint().y, caret_point.y - curCharRect.Height()));
		if (auto position = m_pTextLayout->HitTestPoint(point); position >= 0) {
			if (shift) {
				MoveCaretWithShift(position, point);
				*e.HandledPtr = TRUE;
			} else {
	...
	case VK_DOWN:
	{
		...
		auto caret_point = *Caret->Point;
		auto point = CPointF(caret_point.x,
			(std::min)(m_pTextLayout->HitTestCaretTextPosition(Text->size()).CenterPoint().y, caret_point.y + curCharRect.Height()));
		if (auto newPos = m_pTextLayout->HitTestCaretPoint(point); newPos >= 0) {
			if (shift) {
				MoveCaretWithShift(newPos, point);
				*e.HandledPtr = TRUE;
			} else {
				MoveCaret(newPos, point);
				*e.HandledPtr = TRUE;
			}
		}
		break;
	...
	case VK_HOME:
	{
		auto [index, rect] = m_pTextLayout->HitTestParagraphFirstPosition(*Caret->Current);
		auto caret_point = rect.CenterPoint();
		if (shift) {
	...
	case VK_END:
	{
		auto [index, rect] = m_pTextLayout->HitTestParagraphLastPosition(*Caret->Current);
		auto caret_point = rect.CenterPoint();
		if (shift) {
			MoveCaretWithShift(index, caret_point);
			*e.HandledPtr = TRUE;
	...
	{
		if (ctrl) {
			Caret.get_unconst()->Select(0, Text->size(), m_pTextLayout->HitTestTextPosition(Text->size()).CenterPoint());
			StartCaretBlink();
			*e.HandledPtr = TRUE;
		}
		break;
...
CPointF CTextBox::WndToTextLayout(const CPointF& point) const
{
	return point.OffsetCopy(-ContentRect().left + m_pHScroll->GetScrollPos(), - ContentRect().top + m_pVScroll->GetScrollPos());
}

CPointF CTextBox::TextLayoutToWnd(const CPointF& point) const
{
	return point.OffsetCopy(ContentRect().left - m_pHScroll->GetScrollPos(), ContentRect().top - m_pVScroll->GetScrollPos());
}

CRectF CTextBox::HitTestTextPosition(UINT32 position) const
{
	auto rect = m_pTextLayout->HitTestTextPosition(position);
	return rect.OffsetRectCopy(ContentRect().left - m_pHScroll->GetScrollPos(), ContentRect().top - m_pVScroll->GetScrollPos());
}

size_t CTextBox::HitTestCaretPoint(CPointF point) const
{
	return m_pTextLayout->HitTestCaretPoint(WndToTextLayout(point));
}
...
void CTextBox::Normal_LButtonDblClk(const LButtonDblClkEvent& e)
{
	...
	if (index < Text->size() && std::find(delimiters.begin(), delimiters.end(), Text->at(index)) == delimiters.end()) {
		size_t selBegin = index - 1;
...
{
	auto index = HitTestCaretPoint(e.PointInWnd);
	auto caret_point = m_pTextLayout->HitTestCaretTextPosition(index).CenterPoint();
...
{
	Text.set(m_initText);
	Caret.get_unconst()->Move(Text->size(), m_pTextLayout->HitTestTextPosition(Text->size()).CenterPoint());
...
{
	Caret.get_unconst()->Select(0, Text->size(), Text->empty() ? CPointF() : m_pTextLayout->HitTestTextPosition(Text->size()).CenterPoint());
}

void CTextBox::Deselect()
{
	Caret.get_unconst()->Select(0, 0, Text->empty() ? CPointF() : m_pTextLayout->HitTestTextPosition(0).CenterPoint());
}
...
void CTextBox::PaintText(const PaintEvent& e)
{
	...
	for (auto iter = m_pTextLayout->Paragraphs->begin(); iter != m_pTextLayout->Paragraphs.get_unconst()->end(); ++iter) {
		auto top = height;
		auto bottom = height + (*iter)->GetHeight();
		auto page_top = m_pVScroll->GetScrollPos();
		auto page_bottom = page_top + ContentRect().Height();
		if ((page_top <= top && top <= page_bottom) || (page_top <= bottom && bottom <= page_bottom)) {
			if ((*iter)->GetTextLayoutPtr()) {
				GetWndPtr()->GetDirectPtr()->GetD2DDeviceContext()->DrawTextLayout(
					CPointF(0, height), (*iter)->GetTextLayoutPtr(), GetWndPtr()->GetDirectPtr()->GetColorBrush(GetFormat().Color), D2D1_DRAW_TEXT_OPTIONS::D2D1_DRAW_TEXT_OPTIONS_CLIP);
			}
		}
	...
	//	GetWndPtr()->GetDirectPtr()->DrawSolidRectangle(SolidLine(1.f, 0.f, 0.f, 1.f, 1.f), m_pText->HitTestSelectTextPosition(i));
	//}
	...
	for (size_t i = 0; i < Text->size(); i++) {
		switch (Text->at(i)) {
			...
			case L'\n':
			{
				GetWndPtr()->GetDirectPtr()->DrawLineFeed(GetBlankLine(), m_pTextLayout->HitTestSelectTextPosition(i));
				break;
			}
			case L'\t':
			{
				GetWndPtr()->GetDirectPtr()->DrawTab(GetBlankLine(), m_pTextLayout->HitTestTextPosition(i));
				break;
			}
			case L' ':
			{
				GetWndPtr()->GetDirectPtr()->DrawHalfSpace(GetBlankLine(), m_pTextLayout->HitTestTextPosition(i));
				break;
			}
			case L'　':
			{
				GetWndPtr()->GetDirectPtr()->DrawFullSpace(GetBlankLine(), m_pTextLayout->HitTestTextPosition(i));
				break;
			}
			default:
				break;
...
{
	auto rects = m_pTextLayout->HitTestSelectTextRange(*Caret->SelectedBegin, *Caret->SelectedEnd - *Caret->SelectedBegin);
	auto fill = GetIsFocused() ? GetSelectedOverlay() : GetUnfocusSelectedOverlay();

	for (auto iter = rects.cbegin(); iter != rects.cend(); ++iter) {
		GetWndPtr()->GetDirectPtr()->FillSolidRectangle( fill, *iter);
	}
}
...
{
	//Draw Caret
	CRectF caretRect = m_pTextLayout->HitTestCaretTextPosition(*Caret->Current);
		...
		if (compositionInfo.DisplayAttribute.lsStyle != TF_LS_NONE) {
			...
			for (auto n = compositionInfo.Start; n < compositionInfo.End; n++) {
				...
				if (n == compositionInfo.Start/* || (charRects[n - 1].bottom + charRects[n - 1].Height() / 2.f) < charRects[n].bottom*/) {
					auto rect = m_pTextLayout->HitTestTextPosition(n);
					ptStart.SetPoint(rect.left, rect.bottom);
				}
				if (n == (compositionInfo.End - 1)/* || (charRects[n].bottom + charRects[n].Height() / 2.f) < charRects[n + 1].bottom*/) {
					auto rect = m_pTextLayout->HitTestTextPosition(n);
					ptEnd.SetPoint(rect.right, rect.bottom);
