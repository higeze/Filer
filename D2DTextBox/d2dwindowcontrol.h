...
// 典型的なグループボタン
class D2DGroupButton : public D2DControl
{
	public :
		D2DGroupButton(){};
		...
		void SetTitle( int idx, LPCWSTR title );
		void Clear(){ clickced_btn_idx_ = -1; }
		int GetSelectedIdx(){ return clickced_btn_idx_; }
	private :
		void DrawButton( D2DContext& cxt );
		int btncnt_;
		int clickced_btn_idx_;

		D2D1_COLOR_F clr_fore_;


		std::shared_ptr<std::wstring> titles_;

};


// これ何？
class D2DButtonGruop : public D2DControl
{
	public :
		D2DButtonGruop(){};
		virtual void MeToLast(){};
...
class D2DMessageBox : public D2DControls
{
	...
	public :
		static D2DMessageBox* ShowDialog( D2DControl* parent, LPCWSTR msg, LPCWSTR title, int btntype );
...
class D2DStackPanel : public D2DControls
{
	public :
		enum TYP { VERTICAL,HORIZON,CHILD_VERTICAL };
		D2DStackPanel():typ_(VERTICAL){};
		D2DStackPanel( TYP ty ):typ_(ty){}
		virtual void SetRect( const FRectF& rc );
...
{
	public :
		D2DDebugControl():wakuclr_(D2RGB(0,0,0)),fillclr_(D2RGB(255,255,255)){}
		D2DDebugControl(ColorF waku):wakuclr_(waku),fillclr_(D2RGB(255,255,255)){}
		D2DDebugControl(ColorF waku,ColorF fill):wakuclr_(waku),fillclr_(fill){}
		...
		ColorF wakuclr_, fillclr_; // frame color
	protected :
		virtual void OnCreate();
...
class D2DTabControls : public D2DControls
{
	public :
		...
		void OnTagDraw( D2DContext& cxt );
		FRectF TagDrawRect();

		virtual FRectF GetContentRect();
		D2DControls* Child( UINT idx );
		D2DControls* Replace( UINT idx, LPCWSTR typ_name );
